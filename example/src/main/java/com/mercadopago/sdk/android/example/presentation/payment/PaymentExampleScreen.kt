...
private sealed interface PaymentExampleState {
    ...
    data class Success(val orderStatus: String) : PaymentExampleState
}
                    ...
                    checkout.show { result ->
                        when (result) {
                            is MercadoPagoCheckoutResult.Success ->
                                state = PaymentExampleState.Success(result.paymentData.orderStatus)
                            ...
                            is MercadoPagoCheckoutResult.UserCancelled -> {
                                val screensInfo = result.cancelledData.screens
...
private fun PaymentIdleContent(
    ...
    Column(
        ...
        Text(
            text = "Inicie um pagamento usando MPCheckoutType.Payment",
            fontSize = 14.sp,
...
private fun PaymentSuccessPreview() {
    MercadoPagoSampleTheme {
        PaymentSuccessContent(orderStatus = "approved")
    }
}
