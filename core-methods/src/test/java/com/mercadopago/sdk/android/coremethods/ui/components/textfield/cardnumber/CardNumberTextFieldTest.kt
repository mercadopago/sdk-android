...
internal class CardNumberTextFieldTest {
    ...
    @Test
    fun `when field is empty Then input should be empty`() = runTest {
        // Given
        val input = ""
    ...
    @Test
    fun `when card number has 15 digits and mask is default Then input should be formatted`() = runTest {
        // Given
        ...
        events.receiveAsFlow().test {
            ...
            assertEquals(CardNumberTextFieldEvent.OnBinChanged(cardBin = input.take(BinLength)), awaitItem())
            assertEquals(
                CardNumberTextFieldEvent.OnLastFourDigitsFilled(lastFourDigits = input.takeLast(LastDigitsLength)),
                awaitItem()
            )
    ...
    @Test
    fun `when card number has 15 digits and mask is custom Then input should be formatted`() = runTest {
        // Given
        val input = "378282246310005"
    ...
    @Test
    fun `when card number has 16 digits and mask is default Then input should be formatted`() = runTest {
        // Given
    ...
    @Test
    fun `when card number has 19 digits and mask is default Then input should be formatted`() = runTest {
        // Given
    ...
    @Test
    fun `when card number has 14 digits and mask is default Then input should be formatted`() = runTest {
        // Given
    ...
    @Test
    fun `when card number more digits than the max length Then input should only have max length`() = runTest {
        // Given
        ...
        events.receiveAsFlow().test {
            ...
            assertEquals(
                CardNumberTextFieldEvent.OnLastFourDigitsFilled(
                    lastFourDigits = inputWithMaxLength.takeLast(LastDigitsLength),
                ),
                awaitItem(),
            )
            assertEquals(CardNumberTextFieldEvent.IsValid(isValid = true), awaitItem())
    ...
    @Test
    fun `when user types number and deletes bin Then event should trigger update`() = runTest {
        // Given
        val input = "36227206271667"
        val events = Channel<CardNumberTextFieldEvent>(Channel.BUFFERED)
