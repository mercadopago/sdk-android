...
internal class CoreMethodsErrorObservability(
    ...
    internal fun ResultError.toNativeErrorCode(): NativeErrorCode =
        when (this) {
            ...
            is ResultError.Request -> when {
                code.uppercase() in TIMEOUT_CODES -> NativeErrorCode.REQUEST_TIMEOUT
                code.uppercase() in CONNECTION_CODES -> NativeErrorCode.CONNECTION_UNAVAILABLE
    ...
    private fun ResultError.toDiagnostic(): NativeErrorDiagnostic? =
        when (this) {
            ...
            is ResultError.Request -> when {
                code.uppercase() in TIMEOUT_CODES -> NativeErrorDiagnostic.TIMEOUT
                code.uppercase() in CONNECTION_CODES -> NativeErrorDiagnostic.OFFLINE
