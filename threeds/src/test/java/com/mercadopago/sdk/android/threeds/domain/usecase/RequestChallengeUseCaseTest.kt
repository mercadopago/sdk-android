...
internal class RequestChallengeUseCaseTest {
    ...
    @Test
    fun `when challenge succeeds with AUTHORIZED response Then should call onSuccess without challenge`() =
        runTest {
            // Given
            ...
            every { authenticateUseCase(any()) } returns flowOf(mockAuthResponse)
            ...
            coVerify {
                threeDSSDKAdapter.createTransaction(
                    MPThreeDSDirectoryServer.paymentMethodDirectoryServer(paymentMethodId)
                )
            }
            ...
            verify {
                authenticateUseCase(
                    params = ThreeDSAuthenticationParams(
                        cardToken,
                        mockAuthParams.sdkAppId,
                        mockAuthParams.deviceData,
                        mockAuthParams.sdkEphemeralPublicKey,
                        "10",
                        mockAuthParams.sdkReferenceNumber,
                        mockAuthParams.sdkTransactionId
                    )
                )
            }
            ...
            assertEquals(
                mockAuthResponse.acsReferenceNumber,
                capturedResult.challengeResponse.acsReferenceNumber
            )
        }
    ...
    @Test
    fun `when getAuthenticationRequestParameters returns null Then should complete without errors`() =
        runTest {
            // Given
            val cardToken = "test_token"
            val paymentMethodId = "visa"
    ...
    @Test
    fun `when GeneralSecurityException occurs Then should call onError with security error`() =
        runTest {
            // Given
            val cardToken = "test_token"
            val paymentMethodId = "visa"
