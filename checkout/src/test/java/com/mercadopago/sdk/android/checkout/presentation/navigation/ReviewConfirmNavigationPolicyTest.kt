...
internal class ReviewConfirmNavigationPolicyTest {
    ...
    @Test
    fun `load failure returns to card transaction with error for every origin`() {
        val event = ReviewConfirmViewEvent.OnLoadFailure(checkoutError())

        origins.forEach { origin ->
            assertSame(
                ReviewConfirmNavigationAction.ReturnToCardTransactionWithGenericError,
                resolve(event, cardTransaction, origin),
            )
        }
    }

    @Test
    fun `load failure navigates up preserving origin for fallback checkout types`() {
        val event = ReviewConfirmViewEvent.OnLoadFailure(checkoutError())
