...
internal class ReviewConfirmViewModelTest {
    ...
    private fun makeViewModel(
        ...
        analyticsTracker: ReviewConfirmAnalyticsTracker = mockk(relaxed = true),
    ...
    fun `Success state maps footer correctly`() = runTest {
        ...
        assertEquals(BigDecimal("300.00"), state.footer.totalAmount)
    }
    ...
    @Test
    fun `init tracks impression`() = runTest {
        // Given
        val tracker = mockk<ReviewConfirmAnalyticsTracker>(relaxed = true)
        coEvery { fetchReviewConfirmUseCase(any()) } returns Result.Success(viewData)

        // When
        makeViewModel(analyticsTracker = tracker)

        // Then
        verify { tracker.trackImpression() }
    }

    @Test
    fun `onConfirmClicked tracks continue`() = runTest {
        // Given
        val tracker = mockk<ReviewConfirmAnalyticsTracker>(relaxed = true)
        coEvery { fetchReviewConfirmUseCase(any()) } returns Result.Success(viewData)
        coEvery { processOrderUseCase(any()) } returns Result.Success(
        ...
        val viewModel = makeViewModel(analyticsTracker = tracker)

        // When
        viewModel.onConfirmClicked()

        // Then
        verify { tracker.trackContinue() }
    }

    @Test
    fun `onConfirmClicked does not track continue when state is not Success`() = runTest {
        // Given
        val tracker = mockk<ReviewConfirmAnalyticsTracker>(relaxed = true)
        val deferred = CompletableDeferred<Result<ReviewConfirmViewData, MercadoPagoCheckoutError>>()
        coEvery { fetchReviewConfirmUseCase(any()) } coAnswers { deferred.await() }
        ...
        // Then
        verify(exactly = 0) { tracker.trackContinue() }
        deferred.complete(Result.Success(viewData))
    }

    @Test
    fun `onBackPressed tracks back`() = runTest {
        // Given
        val tracker = mockk<ReviewConfirmAnalyticsTracker>(relaxed = true)
        coEvery { fetchReviewConfirmUseCase(any()) } returns Result.Success(viewData)
        val viewModel = makeViewModel(analyticsTracker = tracker)
        ...
        // Then
        verify { tracker.trackBack() }
    }

    @Test
    fun `onModifyPaymentMethodClicked tracks payment method changed`() = runTest {
        // Given
        val tracker = mockk<ReviewConfirmAnalyticsTracker>(relaxed = true)
        coEvery { fetchReviewConfirmUseCase(any()) } returns Result.Success(viewData)
        val viewModel = makeViewModel(analyticsTracker = tracker)
        ...
        // Then
        verify { tracker.trackPaymentMethodChanged() }
    }

    @Test
    fun `onModifyEmailClicked tracks payer field changed`() = runTest {
        // Given
        val tracker = mockk<ReviewConfirmAnalyticsTracker>(relaxed = true)
        coEvery { fetchReviewConfirmUseCase(any()) } returns Result.Success(viewData)
        val viewModel = makeViewModel(analyticsTracker = tracker)
        ...
        // Then
        verify { tracker.trackPayerFieldChanged() }
    }
}
