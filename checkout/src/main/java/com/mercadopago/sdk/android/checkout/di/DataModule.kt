...
internal fun provideDataModule() =
    module {
        ...
        factory<ReviewConfirmRepository> { (clientToken: String) ->
            ReviewConfirmRepositoryImpl(dataSource = get(), clientToken = clientToken)
        }
        factory { (clientToken: String) ->
            FetchReviewConfirmUseCase(repository = get { parametersOf(clientToken) })
        }
        ...
        viewModel { (request: ReviewConfirmRequest, clientToken: String) ->
            ReviewConfirmViewModel(
                request = request,
                fetchReviewConfirmUseCase = get { parametersOf(clientToken) },
            )
        }
    }
