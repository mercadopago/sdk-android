...
import com.mercadopago.sdk.android.checkout.presentation.extensions.checkoutGraphViewModel
import com.mercadopago.sdk.android.checkout.presentation.extensions.checkoutGraphViewModels
...
internal fun CheckoutController(
    ...
    NavHost(
        ...
        composable<CheckoutDestination.Loading> {
            LoadingScreen()
            LaunchedEffect(Unit) {
                navController.navigate(CheckoutDestination.Payment) {
        ...
        composable<CheckoutDestination.Payment> { backStackEntry ->
            val (coordinator, paymentBrickViewModel) =
                navController.checkoutGraphViewModels<PaymentBrickViewModel>(backStackEntry) {
        ...
        composable<CheckoutDestination.SecurityCode> { backStackEntry ->
            ...
            val config = coordinatorState.securityCodeConfig ?: run {
                CheckoutRecoveryDestination(onRecover = { navController.popBackStack() })
                return@composable
            }
            LaunchedEffect(Unit) {
            ...
            SecurityCodeScreenDestination(
                ...
                onTokenSuccess = { cardId, token ->
                    coordinator.clearSecurityCodeConfig()
        ...
        composable<CheckoutDestination.Form> { backStackEntry ->
            val (coordinator, cardPaymentViewModel) =
                navController.checkoutGraphViewModels<CardPaymentViewModel>(backStackEntry) {
                    parametersOf(checkoutConfiguration)
                } ?: return@composable
            CardFormScreenDestination(
                cardPaymentViewModel = cardPaymentViewModel,
                onNavigateToInstallments = { installmentData, paymentData ->
                    coordinator.setInstallmentData(installmentData, paymentData)
        ...
        composable<CheckoutDestination.ReviewConfirm> { backStackEntry ->
            val coordinator: CheckoutCoordinatorViewModel =
                navController.checkoutGraphViewModel(backStackEntry) ?: return@composable
            val coordinatorState by coordinator.state.collectAsState()
            ...
            ReviewConfirmScreenDestination(
                ...
                checkoutConfiguration = checkoutConfiguration,
                onBackClick = {
                    coordinator.clearReviewConfirmParams()
                    navController.popBackStack()
                },
                onDismiss = {
                    coordinator.clearReviewConfirmParams()
        ...
        composable<CheckoutDestination.Installment> { backStackEntry ->
            ...
            val coordinatorState by coordinator.state.collectAsState()
            val installmentData = coordinatorState.installmentData
            val paymentData = coordinatorState.paymentData
            if (installmentData == null || paymentData == null) {
                CheckoutRecoveryDestination(onRecover = { navController.popBackStack() })
                return@composable
            }
            ...
            LaunchedEffect(cardPaymentViewEvent) {
                when (val event = cardPaymentViewEvent) {
                    is CardPaymentViewEvent.OnPaymentConfirmed -> {
                        ...
                        coordinator.clearInstallmentData()
                        coordinator.setReviewConfirmParams(event.params)
                        navController.navigate(CheckoutDestination.ReviewConfirm)
                    }
                    else -> Unit
                }
        ...
        composable<CheckoutDestination.OfflineMethodSelector> { backStackEntry ->
            ...
            val coordinatorState by coordinator.state.collectAsState()
            val screenData = coordinatorState.methodSelectionData ?: run {
                CheckoutRecoveryDestination(onRecover = { navController.popBackStack() })
                return@composable
            }
                    ...
                    coordinatorState.methodSelectionData?.let { currentData ->
                        ...
                        checkoutConfiguration.buildProcessOrderParamsForMethodSelection(
                            ...
                            if (currentData.selectionType == SelectionDisplayType.Chevron) {
                                coordinator.clearMethodSelectionData()
                                coordinator.setReviewConfirmParams(params)
                                navController.navigate(CheckoutDestination.ReviewConfirm)
                            } else {
                                coordinator.clearMethodSelectionData()
...
@Composable
private fun CheckoutRecoveryDestination(
    onRecover: () -> Unit,
) {
    LoadingScreen()
    LaunchedEffect(Unit) {
