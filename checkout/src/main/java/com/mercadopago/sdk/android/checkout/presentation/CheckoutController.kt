    ...
    NavHost(
        ...
        composable<CheckoutDestination.ReviewConfirm> { backStackEntry ->
            ...
            ReviewConfirmScreenDestination(
                ...
                onLoadFailure = { error ->
                    pendingReviewConfirmParams = null
                    handleReviewConfirmLoadFailure(
                        error = error,
                        checkoutType = checkoutConfiguration?.checkoutType,
                        cardPaymentViewModel = cardPaymentViewModel,
                        paymentBrickViewModel = paymentBrickViewModel,
            ...
            LaunchedEffect(cardPaymentViewEvent) {
                when (val event = cardPaymentViewEvent) {
                    is CardPaymentViewEvent.OnPaymentConfirmed -> {
                        ...
                        pendingReviewConfirmParams = event.params
                        navController.navigate(CheckoutDestination.ReviewConfirm)
                    }
                    else -> Unit
                }
        ...
        composable<CheckoutDestination.OfflineMethodSelector> { backStackEntry ->
            ...
            MethodSelectionScreenDestination(
                ...
                onOptionSelected = { option ->
                    pendingMethodSelectionData?.let { currentData ->
...
private fun ReviewConfirmScreenDestination(
    ...
    onDismiss: () -> Unit,
    onNavigateBack: () -> Unit,
    onLoadFailure: (MercadoPagoCheckoutError) -> Unit,
) {
    ...
    LaunchedEffect(viewEvent) {
        ...
        when (event) {
            ...
            is ReviewConfirmViewEvent.OnLoadFailure -> onLoadFailure(event.error)
...
private fun handleReviewConfirmLoadFailure(
    ...
    when (checkoutType) {
        is MPCheckoutType.CardTransaction -> {
            cardPaymentViewModel.onReviewConfirmLoadFailure()
            popBackStack()
        }
        is MPCheckoutType.Payment -> {
            paymentBrickViewModel.onReviewConfirmLoadFailure()
            popBackStack()
        }
        else -> {
            popBackStack()
