    ...
    LaunchedEffect(viewEvent) {
        when (val event = viewEvent) {
            is ReviewConfirmViewEvent.OnPaymentSuccess -> {
                viewModel.onViewEventConsumed()
                onDismiss()
                val payment = event.toPaymentData(params, checkoutConfiguration.isCardTransaction())
                CheckoutCallbackHolder.notify(MercadoPagoCheckoutResult.Success(payment))
            }
...
private fun ReviewConfirmViewEvent.OnPaymentSuccess.toPaymentData(
    params: ProcessOrderParams,
