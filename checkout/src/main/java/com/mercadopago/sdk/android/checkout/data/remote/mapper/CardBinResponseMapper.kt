...
internal fun CardBinResponse.toDomain(): CardBinData = CardBinData(
    id = id,
...
private fun CardNumberConfigResponse.toDomain(): CardNumberConfig = CardNumberConfig(
    length = length,
    validation = validation,
    mask = mask,
)

private fun SecurityCodeConfigResponse.toDomain(): BinSecurityCodeConfig = BinSecurityCodeConfig(
    mode = mode,
    length = length,
    cardLocation = cardLocation,
)

private fun IssuerResponse.toDomain(): BinIssuer = BinIssuer(
    id = id,
    name = name,
    secureThumbnail = secureThumbnail,
)

private fun QuotaResponse.toDomain(): Quota = Quota(
    quantity = quantity,
...
private fun TranslationsResponse.toDomain(): CardFormTranslations = CardFormTranslations(
    cardNumber = cardNumber?.toDomain(),
...
private fun FieldTranslationResponse.toDomain(): FieldTranslation = FieldTranslation(
    label = label,
...
private fun FieldErrorTranslationResponse.toDomain(): FieldErrorTranslation = FieldErrorTranslation(
    invalid = invalid,
    incomplete = incomplete,
)

private fun SecurityCodeTranslationResponse.toDomain(): SecurityCodeFieldTranslation = SecurityCodeFieldTranslation(
    label = label,
    placeholder = placeholder,
    helper = helper,
    tooltip = tooltip,
...
private fun InstallmentsTranslationResponse.toDomain(): InstallmentsFieldTranslation = InstallmentsFieldTranslation(
    label = label,
