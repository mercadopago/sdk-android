...
internal class GetSiteIdUseCaseTest {

    private val sdkInitializationRepository = mockk<SdkInitializationRepository>()
    private val useCase = GetSiteIdUseCase(sdkInitializationRepository)
    ...
    fun `when invoke is called with success Then emit siteId`() = runTest {
        ...
        every { sdkInitializationRepository.getSiteId(publicKey) } returns flowOf(siteId)
    ...
    fun `when invoke is called with error Then emit error`() = runTest {
        ...
        val publicKey = "public_key"
        every {
            sdkInitializationRepository.getSiteId(publicKey)
