...
internal class NativeErrorFactoryTest {
    private val factory = NativeErrorFactory()
    ...
    fun `canonical vectors have the approved hash and outputs`() {
        ...
        vectors.forEach { element ->
            ...
            val actual = factory.from(operation, input)
    ...
    fun `neutral input exposes only closed values and sanitizes optionals`() {
        ...
        assertEquals(
            ...
            factory.from(NativeErrorOperation.CARD_FORM_SUBMISSION, input).code,
        )
        assertEquals(
            5,
